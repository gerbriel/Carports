...
- **engineering-service/** — FastAPI (Python) design brain: ASCE 7 loads + member checks + BOM (POC). See its README.
- No root `package.json`. `client/` and `server/` are independent npm packages.

### Engineering source of truth
This repo is the **canonical home of the engineering model**. The sibling
**united-metal-components** repo (company site + material calculator) **mirrors** it —
it must never diverge:
- **Frame-spacing table (Table 4)** is generated by `scripts/gen-frame-spacing.mjs` from
  the gitignored `engineering/` appendix into **both** repos (`client/src/data/frameSpacing.js`
  here + `.../united-metal-components/src/lib/carport/frameSpacing.ts`). Edit the appendix +
  regenerate; never hand-edit the generated files.
- Parametric logic lives in `client/src/data/structural.js` (`deriveStructure`); the BOM/takeoff
  in `client/src/data/components.js`. Full design manual: `engineering/ENGINEERING-SPEC.md` (gitignored).
- **No engineering-firm names** in committed/app files (`engineering/` is gitignored).
